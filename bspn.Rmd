...
```{r}
at <- c(6544011, 3011711)
pa <- c(sum(at),0)
rot <- c(NULL, "Passivo a Descoberto")

bp <- data.frame(rot,"Ativo" = at, "Passivo" = pa)
bp$rot <- c("", "Passivo a Descoberto")

bp2<-bp %>% gather(Ativo, Passivo, key = "Class", value = "Valor")

ggplot(bp2, aes(fill=rot, y=Valor, x=Class)) +
    coord_flip() +
...
dygraph(sh_xts) %>%
  ...
  dyAxis("y",
         ...
         valueFormatter = JS('function(d) {return d+" %"}'),
         axisLabelFormatter = JS('function(d) {return d+" %"}'),
         axisLineColor = "white") %>% 
  dyAxis("x", 
         pixelsPerLabel = 40, 
         gridLineColor = "rgb(222,222,222)", 
         axisLabelFormatter = JS('function(d) {return d.getFullYear()}'), #1
         rangePad = 5,
         axisLineColor = "white") %>% 
