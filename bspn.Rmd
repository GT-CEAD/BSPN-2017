...
plota_sankey <- function(dados, rotulos){
  plot_ly(
    type = "sankey",
    orientation = "h",
    opacity = 0.6,
...
plota_ente <- function(ente_in){
  
  if (ente_in == "Consolidado") {
    ativo_ente <- ativo}
    else {ativo_ente <- ativo %>% filter(ente == ente_in)
  }
  
  lista_nos <- unique(c(levels(ativo_ente$up), levels(ativo_ente$down)))
