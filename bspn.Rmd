...
Em termos de participação de cada tipo de receita e despesa nos entes,
...
plota_percent(recDesp_filtrado_percent_semDiv %>% filter(Ente != "Consolidado"),"Resumo das Receitas e Despesas do BSPN 2017 &mdash exceto dívida")
```
...
* receitas de transferências dos municípios
* despesas com outras despesas correntes na união (recalcular percentuais sem RGPS e Transferências?)
</div>
...
grafico_treemap <- function(dados, titulo) {
  treemap(dados, 
        ...
        type = "color",
        vColor = "Cor",
        fontfamily.title = "Source Sans Pro",
        fontfamily.labels = "Source Sans Pro",
        fontfamily.legend = "Source Sans Pro",
        fontsize.title = 14,
...
Por fim, é interessante comparar a arrecadação, em 2017, dos diversos impostos atribuídos pela Constituição aos entes da Federação.
...
plot_funcoes <- function(funcao){
  ggplot(
    ...
    scale_y_continuous(labels = scales::percent) +
    scale_fill_manual(values = tres_cores) +
    geom_text(aes(label= scales::percent(Valor)),size = 2.5, position = position_stack(vjust = 0.5), hjust = 0.5, family = "Source Sans Pro", color = "white") +
