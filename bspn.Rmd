...
As cores dos ramos entre os níveis dos ativos representam os valores dos <span style="color: white; background-color: #f8ac08; letter-spacing: 0.4em;">municípios</span>, <span style="color: white; background-color: #028063; letter-spacing: 0.4em; color: white;">estados</span> e <span style="color: white; background-color: #6e287c; letter-spacing: 0.4em; color: white;">união</span>.
...
ggplot(recDesp_formatado, aes(fill = Classificador, y = Valor, x = Classe)) +
  coord_flip() +
  geom_bar(stat="identity", width=0.6, color = "white", size = 1) + # p/ transformar em %, position="fill"
  geom_text(aes(label= ifelse(Valor == 0, "", format(round(Valor,0), big.mark = ".", decimal.mark = ",")), nudge_x = ifelse(Classe == "Receitas", -5, +5)),
                size = 3, 
                hjust = 0.5, 
                position = position_stack(vjust = 0.5),
                family = "Source Sans Pro", 
...
```{r serie-historica-funcao, message=FALSE, warning=FALSE}
