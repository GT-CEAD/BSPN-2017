...
# acrescentando dados de 2018-2020

sh_fun_2018_2020_raw <- read_excel('./dados-2018-2020/sh-2018-2020.xlsx')

sh_fun_2018_2020 <- sh_fun_2018_2020_raw %>%
  gather('União', 'Estados', 'Municípios', 'Consolidado',  key = 'ente', value = 'valor') %>%
  spread(key = 'Exercício', value = 'valor') %>%
  rename('Nome_funcao' = `Função`) %>%
  left_join(tabela_nomes_funcoes)

sh_fun <- sh_fun %>%
  left_join(sh_fun_2018_2020)

write.csv2(sh_fun %>% select(-Nome_funcao, -Funcao_Maius), 'sh-funcao-2002-2020.csv', fileEncoding = 'UTF-8')
...
sh_fun_spread <- sh_fun %>%
  ...
  mutate_at(vars(`2002`:`2017`), funs(./referencia)) %>%
  select(-referencia) %>%
  gather(key = "Ano", value = "Valor", `2002`:`2017`)
...
plot_funcoes_linhas <- function(funcao) {
  ...
  geom_line(size = 1) +
  geom_text(aes(x = "2017", 
                label = ifelse(Ano == "2017", 
    ...
    labs(
      ...
      subtitle = "Variação das despesas em percentuais do PIB de 2002 a 2017 em relação aos valores de 2002"
    ) +
  scale_y_continuous(labels = scales::percent) +
  scale_color_manual(values = tres_cores) +
  guides(color = FALSE) +
  tema_BSPN() + 
  expand_limits(x = 2017 - 2002 + 4)
}
